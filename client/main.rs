...
async fn run_client(config_path: PathBuf, once: bool) -> anyhow::Result<()> {
    ...
    let x25519_identity =
        encryption::async_fn::load_x25519_identity(&config.client.keys.x25519_identity_path)
            .await?;
...
async fn generate_keys(output: PathBuf) -> anyhow::Result<()> {
    ...
    let x25519_identity_path = output.join("client.x25519");
...
async fn enroll(server: String, token: String, config_dir: PathBuf) -> anyhow::Result<()> {
    ...
    let x25519_identity = encryption::X25519Identity::generate();
    let x25519_identity_path = config_dir.join("client.x25519");
    encryption::async_fn::write_x25519_identity(&x25519_identity_path, &x25519_identity).await?;

    // Prepare enrollment payload
    let payload = EnrollPayload {
        x25519_public_key: x25519_identity.to_recipient().to_string(),
        auth_public_key: BASE64.encode(signing_key.verifying_key().to_bytes()),
    };

    let payload_json = serde_json::to_vec(&payload)?;

    // Encrypt payload with server's X25519 public key
    let server_recipient = encryption::X25519Recipient::from_str(&parsed.server_x25519_recipient)?;
    let encrypted = encryption::encrypt(&server_recipient, payload_json)?;

    // Serialize as CBOR Content for the server to decode
    let encrypted_cbor = encrypted.to_cbor();
    let encrypted_b64 = BASE64.encode(&encrypted_cbor);
